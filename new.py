...
# Function to create a new column 'detail address' for Clean Energy data
...
# Function to generate the 'Company_specification' column
...
# Function to create a Parquet file from Excel
...
# Function to process AI Hub and Clean Energy data
def process_data(aihub_excel_file, clean_energy_excel_file, input_excel_file):
    try:
        # Read the AI Hub data
        aihub_data = pd.read_excel(aihub_excel_file)
        # Create a new column 'detail address' for AI Hub data
        aihub_data['detail address'] = aihub_data.apply(
            lambda row: f"{row['Address']}, {row['City']}, {row['State']}, {row['Country']}, {str(row['Zip code'])}",
            axis=1
        )

        # Read the Clean Energy data
        clean_energy_data = pd.read_excel(clean_energy_excel_file)
...
if __name__ == "__main__":
    # Define the paths to AI Hub and Clean Energy data Excel files
    aihub_excel_file = "AI_HUB_OpenSearch Company List.xlsx"
    clean_energy_excel_file = "Clean_energy_data.xlsx"

    # Process data and generate Excel and Parquet files
    process_data(aihub_excel_file, clean_energy_excel_file, input_excel_file)
